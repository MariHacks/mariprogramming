...
/**
 * Fill blank identity fields from outline text (never clobber non-empty model values).
...
export function withGuessedIdentity(proposals, text) {
	...
	const title = blank(row.title) ? guessed.title : row.title;
	const teacherName = blank(row.teacherName) ? guessed.teacherName : row.teacherName;
	...
	if (blank(section) && courseCode && title && teacherName) {
		section = '01';
	}
	return {
		...row,
...
/**
 * Map evaluation-table due dates onto assessment rows (layout: Due | Type | …).
 * @param {Record<string, unknown> | null | undefined} proposals
 * @param {string} text
 */
export function withGuessedAssessmentDates(proposals, text) {
	...
	if (assessments.length === 0) return row;

	/** @type {Array<{ due: string, type: string, iso: string | null }>} */
	const tableRows = [];
	for (const line of String(text ?? '').split(/\n/)) {
		const trimmed = line.trim();
		if (!trimmed || /Due Date|Option A|EVALUATION|Type\s+Platform/i.test(trimmed)) continue;
		const layout = trimmed.match(
			/^((?:Weekly|As announced|In common evaluation period|(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)?,?\s*(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}))\s{2,}(.+?)\s{2,}/i
		);
		if (layout) {
			const due = layout[1].trim();
			const type = layout[2].replace(/\*+\s*$/, '').trim();
			tableRows.push({ due, type, iso: dueCellToIso(text, due) });
			continue;
		}
		const loose = trimmed.match(
			/^((?:Weekly|As announced|In common evaluation period|(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)?,?\s*(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}))\s+([A-Za-z][A-Za-z0-9 *-]{2,40})$/i
		);
		if (loose) {
			const due = loose[1].trim();
			const type = loose[2].replace(/\*+\s*$/, '').trim();
			tableRows.push({ due, type, iso: dueCellToIso(text, due) });
		}
	}
	...
	const next = assessments.map((item) => {
		...
		// Prefer evaluation-table calendar dues over model mis-alignment.
		if (hit?.iso) current.date = hit.iso;
		else if (hit && !hit.iso) current.date = null;
		return current;
	});
